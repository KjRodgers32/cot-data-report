...
def get_futures_data() -> pd.DataFrame:
      ...
      return cot.cot_year(year= year, cot_report_type='legacy_fut', store_txt=False)
...
def main() -> None:
    ...
    fx_report_data = fx_data[columns]
    fx_final_report_data = fx_report_data[fx_report_data["As of Date in Form YYYY-MM-DD"] == max(fx_report_data['As of Date in Form YYYY-MM-DD'])]
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            with psycopg2.connect(POSTGRES_URL) as psyconn:
                with psyconn.cursor() as cur:
                        ...
                        fx_final_report_data.to_sql(con=engine, name='futures_data',if_exists='append', index=False)
